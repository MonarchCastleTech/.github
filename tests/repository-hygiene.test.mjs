...
const methodologyEvidence = ["CONTRIBUTING.md"];
const quickStartCommands = ["node --test tests/*.test.mjs"];
const architectureIdentifiers = ["profile/","SECURITY.md"];
const thirdPartyExclusions = ["organization names, logos, and trademarks","external screenshots and contributed assets"];
const licenseDecision = "new-mit";

test("MonarchCastleTech/.github exposes the complete repository documentation contract", () => {
  assert.ok(match, "README must include the managed repository-hygiene block");
  assert.ok(contract.includes("monarchcastletech — organization profile"), "README purpose must match the canonical registry");
...
test("MonarchCastleTech/.github keeps every managed image local and ships a valid social preview", () => {
...
test("MonarchCastleTech/.github documents citation, rights, and HTTPS policy", () => {
